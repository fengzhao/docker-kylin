...
extract_info() {
    ...
    echo "$branch $arch $version $release_date $kernel_type $desktop_env $update_type $hardware_type"
}
...
build_image() {
    ...
    local image_tag="${image_prefix}:${branch}-${arch}-${version}-${release_date}-${kernel_type}-${desktop_env}-${update_type}-${hardware_type}"
...
for ISO_FILE in $ISO_FILES; do
    ...
    copy_rootfs
    unmount_iso
    build_image "$BRANCH" "$ARCH" "$VERSION" "$RELEASE_DATE" "$KERNEL_TYPE" "$DESKTOP_ENV" "$UPDATE_TYPE" "$HARDWARE_TYPE"
